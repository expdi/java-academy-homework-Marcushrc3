package expeditors.backend.customer.service;



import expeditors.backend.customer.dao.CustomerDAO;
import expeditors.backend.customer.dao.DAOFactory;
...
public class CustomerService {
    ...
    private final int MINIMUN_AGE = 24;

    private CustomerDAO customerDAO = DAOFactory.customerDAO();

    public Customer addCustomer(Customer c) {

        long age = c.getDob().until(LocalDate.now(), ChronoUnit.YEARS);
        if (age < 18) {
            throw new RuntimeException("Customer age must be at least " + MINIMUN_AGE);
        }
